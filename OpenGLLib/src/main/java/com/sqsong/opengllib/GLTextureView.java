...
public class GLTextureView extends TextureView
    ...
    public void setRenderer(Renderer renderer) {
        ...
        boolean supportsGLES3 = OpenGLUtilsKt.isOpenGL30Supported(getContext());
        if (supportsGLES3) {
            setEGLContextClientVersion(3);
            // 设置适用于 GLES 3.x 的 EGLConfigChooser
            setEGLConfigChooser(new GLES3ConfigChooser(true));
        } else {
            setEGLContextClientVersion(2);
            // 设置适用于 GLES 2.0 的 EGLConfigChooser
    ...
    private class SimpleEGLConfigChooser extends ComponentSizeChooser {
        ...
        public EGLConfig chooseConfig(EGL10 egl, EGLDisplay display, EGLConfig[] configs) {
            ...
            // 如果请求的配置不支持，尝试其他配置或回退
            for (EGLConfig cfg : configs) {
                int r = findConfigAttrib(egl, display, cfg, EGL10.EGL_RED_SIZE, 0);
                int g = findConfigAttrib(egl, display, cfg, EGL10.EGL_GREEN_SIZE, 0);
                int b = findConfigAttrib(egl, display, cfg, EGL10.EGL_BLUE_SIZE, 0);
                int a = findConfigAttrib(egl, display, cfg, EGL10.EGL_ALPHA_SIZE, 0);
                int d = findConfigAttrib(egl, display, cfg, EGL10.EGL_DEPTH_SIZE, 0);
                if (r == 8 && g == 8 && b == 8 && a == 8 && d >= 16) {
                    return cfg;
                }
            }
            return null;
    ...
    private class GLES3ConfigChooser extends BaseConfigChooser {
        public GLES3ConfigChooser(boolean withDepthBuffer) {
            super(new int[]{
                    EGL10.EGL_RED_SIZE, 8,
                    EGL10.EGL_GREEN_SIZE, 8,
                    EGL10.EGL_BLUE_SIZE, 8,
                    EGL10.EGL_ALPHA_SIZE, 8,
                    EGL10.EGL_DEPTH_SIZE, withDepthBuffer ? 16 : 0,
                    EGL10.EGL_RENDERABLE_TYPE, 0x0004, // EGL_OPENGL_ES3_BIT_KHR 的值
                    EGL10.EGL_NONE
            });
        }
        ...
        public EGLConfig chooseConfig(EGL10 egl, EGLDisplay display, EGLConfig[] configs) {
            for (EGLConfig config : configs) {
                ...
                if ((renderableType & 0x0004) != 0) { // 检查是否支持 OpenGL ES 3.x
                    return config;
                }
            }
            return null;
        ...
        public void start() {
            ...
            if (view == null) {
                ...
                if (eglContext == null || eglContext == EGL10.EGL_NO_CONTEXT) {
                    // 如果创建 OpenGL ES 3.x 上下文失败，尝试回退到 OpenGL ES 2.0
                    Log.w(TAG, "Failed to create OpenGL ES 3.x context, trying OpenGL ES 2.0");
            ...
            if (eglContext == null || eglContext == EGL10.EGL_NO_CONTEXT) {
                eglContext = null;
                String openglVersion = "unknown";
                if (view != null) {
                    openglVersion = OpenGLUtilsKt.getDeviceOpenGLVersion(view.getContext());
                }
                throwEglException("create EGL context error, openglVersion=" + openglVersion);
            }
            if (LOG_EGL) {
                Log.w("EglHelper", "createContext " + eglContext + " tid=" + Thread.currentThread().getId());
            }
